...
public class MainActivity extends Activity {
    ...
    void sortItems(){Collections.sort(items,(a,b)->{if(foldersFirst&&a.isDirectory()!=b.isDirectory())return a.isDirectory()?-1:1;int c;if(sort.equals("size"))c=Long.compare(a.length(),b.length());else if(sort.equals("date"))c=Long.compare(a.lastModified(),b.lastModified());else if(sort.equals("type"))c=ext(a).compareToIgnoreCase(ext(b));else if(sort.equals("tag"))c=tagFor(a).compareToIgnoreCase(tagFor(b));else if(sort.equals("parts"))c=Integer.compare(partNumber(a.getName()),partNumber(b.getName()));else if(sort.equals("title"))c=titleFor(a.getName()).compareToIgnoreCase(titleFor(b.getName()));else c=a.getName().compareToIgnoreCase(b.getName());if(c==0)c=a.getName().compareToIgnoreCase(b.getName());return descending?-c:c;});}
    void addFileRow(File f){addFileColumnRow(f);}
    String tagFor(File f){return prefs.getString("tag:"+f.getAbsolutePath(),"");}
    String titleFor(String name){if(name==null)return "";int p=name.lastIndexOf('.');return p>0?name.substring(0,p):name;}
    int partNumber(String name){if(name==null)return Integer.MAX_VALUE;java.util.regex.Matcher m=java.util.regex.Pattern.compile("(?i)(?:part|pt|p)[\\s._-]*(\\d+)").matcher(name);if(m.find())try{return Integer.parseInt(m.group(1));}catch(Exception ignored){}m=java.util.regex.Pattern.compile("(?:^|[\\s._-])(\\d+)(?:$|[\\s._-])").matcher(name);if(m.find())try{return Integer.parseInt(m.group(1));}catch(Exception ignored){}return Integer.MAX_VALUE;}
    ...
    void chooseSort(){String[] x={"Name","Type / Extension","Size","Modified date","Tag","Parts","Title","Reverse order"};new AlertDialog.Builder(this).setTitle("Sort By").setItems(x,(d,w)->{if(w==7)descending=!descending;else if(w==0)sort="name";else if(w==1)sort="type";else if(w==2)sort="size";else if(w==3)sort="date";else if(w==4)sort="tag";else if(w==5)sort="parts";else if(w==6)sort="title";if(currentDir!=null)refreshLocal();else if(currentTree!=null)openCloud(currentTree);});}
